...
bool RRTPlanner::check_if_on_obstacle(TreeNode* rrtnode,TreeNode* node){
    ...
    int k=0;
    while(p<=dist) {
    test->cell.x=rrtnode->cell.x+p*dist*cos(theta);
    test->cell.y=rrtnode->cell.y+p*dist*sin(theta);
        k++;
...
bool  RRTPlanner::getnear(TreeNode* &node){
    near.clear();
    //ROS_ERROR("rrtsixe:%d",rrt_nodes_.size());
    for(int i = 0;i<rrt_nodes_.size();i++){
       // ROS_ERROR("near:%f",hypot(rrt_nodes_[i]->cell.x - node->cell.x, rrt_nodes_[i]->cell.y - node->cell.y));
...
bool RRTPlanner::extendConnectNode(PointCell random)
{
    ...
    if(extendSingleStep(rrt_nodes_0[j], node, random))
    {
        ...
        if(connect!=-1)
        {
            ...
            for(int i=connect;i>=0;i--){
               // rrt_nodes_0[]
                rrt_nodes_1.push_back(rrt_nodes_0[i]);
            }
            return true;
        }
        }
